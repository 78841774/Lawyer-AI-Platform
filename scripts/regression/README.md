...
v6.7 turns the v5.0-v6.6 manual verification flow into local regression scripts. The suite checks backend compilation, frontend build, Personal Alpha status APIs, Case OS core APIs, Case OS quality APIs, v6.8 hardening APIs, v6.9 release candidate APIs, v7.0 personal production APIs, v7.1 AI Gateway APIs, v7.2 Material Runtime APIs, metadata-only response safety, safe not_found behavior, injected/path-like input handling, git safety, runtime ignore rules, docs completeness, and tracked sensitive files.
...
- `check_personal_material_runtime_apis.sh`: checks v7.2 Personal Material Runtime status, providers, parse jobs, OCR jobs, OCR preview, review queue, source traces, audit, and safety metadata.
