...
v6.7 turns the v5.0-v6.6 manual verification flow into local regression scripts. The suite checks backend compilation, frontend build, Personal Alpha status APIs, Case OS core APIs, Case OS quality APIs, metadata-only response safety, safe not_found behavior, injected/path-like input handling, git safety, runtime ignore rules, docs completeness, and tracked sensitive files.
