...
v3.0 adds an Internal Alpha identity and workspace foundation for local demos. v3.1 adds Dev Token / API Key current-user resolution. v3.2 adds JWT Auth Foundation. This is not production authentication.
...
v3.2 adds JWT settings. Production deployments must replace `JWT_SECRET_KEY` and manage it with a secret manager or equivalent deployment secret storage.
...
`JWT_SECRET_KEY=local-dev-secret-change-me` is an example value only. Never use it outside local development.
