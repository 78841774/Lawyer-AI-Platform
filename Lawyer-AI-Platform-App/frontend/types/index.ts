...
export type TrialSafetyBase = {
  owner_only: boolean;
  trial_metadata_only: boolean;
  metadata_only: boolean;
  draft_only: boolean;
  raw_case_content_included: boolean;
  raw_ocr_content_included: boolean;
  api_key_exposed: boolean;
  provider_live_call_triggered: boolean;
  public_link_created: boolean;
  email_sent: boolean;
  external_delivery_triggered: boolean;
  ...
  raw_content_written_to_git_docs_regression: boolean;
};

export type TrialSessionMockRequest = {
  trial_name: string;
  case_mode: string;
  owner_user_id?: string;
  case_reference_label?: string;
  explicit_owner_confirmation: boolean;
  explicit_no_raw_content_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
  explicit_no_training_confirmation: boolean;
  explicit_no_external_delivery_confirmation: boolean;
};

export type TrialSession = TrialSafetyBase & {
  trial_id: string;
  trial_name: string;
  case_mode: string;
  owner_user_id: string;
...
export type TrialSessionList = TrialSafetyBase & {
  trials: TrialSession[];
  trial_count: number;
  warnings: string[];
};

export type TrialReadinessStatus = TrialSafetyBase & {
  enabled: boolean;
  version: string;
  runtime_label: string;
  trial_readiness_ready: boolean;
  trial_checklist_ready: boolean;
  issue_log_ready: boolean;
  quality_review_ready: boolean;
  safety_confirmation_ready: boolean;
  optimization_backlog_ready: boolean;
  live_provider_disabled: boolean;
  open_case_training_disabled: boolean;
  skill_auto_publish_disabled: boolean;
  warnings: string[];
};

export type TrialChecklist = TrialSafetyBase & {
  trial_id: string;
  case_workspace_checked: boolean;
  material_workspace_checked: boolean;
  ocr_status_checked: boolean;
  fact_preview_checked: boolean;
  fact_correction_checked: boolean;
  legal_draft_checked: boolean;
  skill_final_drafts_checked: boolean;
  owner_output_center_checked: boolean;
  source_trace_checked: boolean;
  review_queue_checked: boolean;
  trust_safety_checked: boolean;
  diagnostics_collapsed_checked: boolean;
  checked_item_count: number;
  warnings: string[];
};

export type StageObservationMockRequest = {
  stage_id: string;
  notes?: string;
  issue_count?: number;
};

export type StageObservation = TrialSafetyBase & {
  trial_id: string;
  stage_id: string;
  stage_name: string;
...
export type StageObservationList = TrialSafetyBase & {
  trial_id: string;
...
export type IssueLogMockRequest = {
  stage_id?: string;
  issue_type?: string;
  severity?: string;
  title: string;
...
export type IssueLogItem = TrialSafetyBase & {
  issue_id: string;
  trial_id: string;
  stage_id: string;
  issue_type: string;
  ...
  suggested_fix: string;
  status: string;
  blocks_trial: boolean;
  warnings: string[];
};

export type IssueLogList = TrialSafetyBase & {
...
export type QualityReview = TrialSafetyBase & {
  review_id: string;
  trial_id: string;
  overall_score: number;
  stage_scores: Record<string, number>;
  fact_quality_score: number;
  legal_draft_quality_score: number;
  skill_helpfulness_score: number;
  source_trace_score: number;
  owner_download_score: number;
  optimization_suggestions: string[];
  warnings: string[];
};

export type SafetyConfirmation = TrialSafetyBase & {
  trial_id: string;
  owner_only_confirmed: boolean;
  draft_only_confirmed: boolean;
  no_public_link_confirmed: boolean;
  no_email_confirmed: boolean;
  no_external_delivery_confirmed: boolean;
  no_final_legal_opinion_confirmed: boolean;
  no_final_report_confirmed: boolean;
  no_open_case_training_confirmed: boolean;
  no_skill_auto_publish_confirmed: boolean;
  no_api_key_exposed_confirmed: boolean;
  no_raw_content_in_diagnostics_confirmed: boolean;
  no_raw_content_in_git_docs_regression_confirmed: boolean;
  warnings: string[];
};

export type OptimizationBacklogMockRequest = {
...
export type OptimizationBacklogItem = TrialSafetyBase & {
  backlog_id: string;
  source_trial_id: string;
  source_issue_ids: string[];
  priority: string;
...
export type OptimizationBacklogList = TrialSafetyBase & {
  backlog_items: OptimizationBacklogItem[];
  backlog_count: number;
  warnings: string[];
};

export type TrialAuditTimeline = TrialSafetyBase & {
  events: Array<Record<string, unknown>>;
  event_count: number;
  warnings: string[];
};

export type TrialSafetyStatus = TrialSafetyBase & {
  safety_items: string[];
  safety_item_count: number;
  all_safety_checks_passed: boolean;
  warnings: string[];
...
export type ProviderReadinessSafetyBase = {
  metadata_only: boolean;
  dry_run: boolean;
  provider_gated: boolean;
  live_default_enabled: boolean;
  live_call_allowed: boolean;
  live_call_executed: boolean;
  api_key_exposed: boolean;
  secret_value_returned: boolean;
  secret_logged: boolean;
  frontend_key_input_enabled: boolean;
  external_transfer_triggered: boolean;
...
export type ProviderMetadata = ProviderReadinessSafetyBase & {
  provider_id: string;
  ...
  usage_meter_supported: boolean;
  cost_metadata_supported: boolean;
  adapter_registered: boolean;
  status: string;
  warnings: string[];
};

export type ProviderList = ProviderReadinessSafetyBase & {
  providers: ProviderMetadata[];
  provider_count: number;
  key_loaded_count: number;
  dry_run_ready_count: number;
  live_disabled_count: number;
  blocked_provider_count: number;
  warnings: string[];
};

export type ProviderStatus = ProviderReadinessSafetyBase & {
  enabled: boolean;
  version: string;
  runtime_label: string;
  provider_readiness_ready: boolean;
  provider_registry_ready: boolean;
  secret_boundary_ready: boolean;
  live_gate_ready: boolean;
  usage_cost_metadata_ready: boolean;
  dry_run_health_ready: boolean;
  real_provider_calls_still_disabled: boolean;
  provider_count: number;
  key_loaded_count: number;
  warnings: string[];
};

export type CategorySummary = ProviderReadinessSafetyBase & {
  category: string;
  provider_count: number;
  key_loaded_count: number;
  dry_run_ready_count: number;
...
export type CategorySummaryList = ProviderReadinessSafetyBase & {
  categories: CategorySummary[];
  category_count: number;
  warnings: string[];
};

export type SecretBoundaryStatus = ProviderReadinessSafetyBase & {
  provider_id: string;
  key_env_names: string[];
  requires_api_key: boolean;
  key_loaded: boolean;
  key_source: string;
  key_value_exposed: boolean;
  key_prefix_returned: boolean;
  key_suffix_returned: boolean;
  masked_key_returned: boolean;
  token_value_returned: boolean;
  secret_value_stored: boolean;
  warnings: string[];
};

export type LiveGateMockRequest = {
  provider_id: string;
  explicit_live_confirmation?: boolean;
  owner_authorized?: boolean;
  external_transfer_acknowledged?: boolean;
  no_training_use_acknowledged?: boolean;
  audit_acknowledged?: boolean;
};

export type LiveGateStatus = ProviderReadinessSafetyBase & {
  gate_id: string;
  provider_id: string;
...
export type LiveGateList = ProviderReadinessSafetyBase & {
  live_gates: LiveGateStatus[];
  live_gate_count: number;
  warnings: string[];
};

export type UsagePolicy = ProviderReadinessSafetyBase & {
  provider_id: string;
  ...
  billable_call_executed: boolean;
  usage_recorded_as_metadata_only: boolean;
  warnings: string[];
};

export type ProviderHealthDryRun = ProviderReadinessSafetyBase & {
  provider_id: string;
  ...
  live_blocked_reason: string;
  next_required_confirmation: string;
  network_call_executed: boolean;
  warnings: string[];
};

export type ProviderAuditTimeline = ProviderReadinessSafetyBase & {
  events: Array<Record<string, unknown>>;
  event_count: number;
  warnings: string[];
};

export type ProviderSafetyStatus = ProviderReadinessSafetyBase & {
  safety_items: string[];
  safety_item_count: number;
  all_safety_checks_passed: boolean;
  warnings: string[];
...
export type PersonalProductionProviderCapabilities = {
  ...
  personal_trial_readiness_status: string;
  personal_provider_readiness_status: string;
  personal_material_live_connection_status: string;
  personal_live_connection_status: string;
  personal_legal_enterprise_gateway_status: string;
  ...
  skill_package_versioning_status: string;
  internal_training_experience_package_status: string;
  practice_load_review_status: string;
  ...
  trial_readiness_ready: boolean;
  trial_checklist_ready: boolean;
  trial_issue_log_ready: boolean;
  trial_quality_review_ready: boolean;
  trial_safety_confirmation_ready: boolean;
  trial_optimization_backlog_ready: boolean;
  trial_issue_log_reference_only: boolean;
  trial_quality_review_reference_only: boolean;
  provider_readiness_ready: boolean;
  provider_registry_ready: boolean;
  secret_boundary_ready: boolean;
  live_gate_ready: boolean;
  ...
  dry_run_health_ready: boolean;
  real_provider_calls_still_disabled: boolean;
  material_live_connection_ready: boolean;
  material_live_provider_registry_ready: boolean;
  material_live_secret_boundary_ready: boolean;
  material_live_gate_ready: boolean;
  material_live_dry_run_health_ready: boolean;
  material_live_raw_content_blocked: boolean;
  material_live_ai_prompt_injection_blocked: boolean;
  personal_live_connection_ready: boolean;
  personal_live_connection_provider_registry_ready: boolean;
  personal_live_connection_secret_boundary_ready: boolean;
  personal_live_connection_gate_ready: boolean;
  personal_live_connection_usage_cost_ready: boolean;
  personal_live_connection_health_ready: boolean;
  personal_live_connection_audit_ready: boolean;
  legal_enterprise_gateway_ready: boolean;
  legal_provider_readiness_ready: boolean;
  enterprise_provider_readiness_ready: boolean;
  legal_source_trace_ready: boolean;
  enterprise_verification_ready: boolean;
  legal_enterprise_review_required: boolean;
  training_artifact_loader_ready: boolean;
  codex_training_scheme_ready: boolean;
  case_cause_taxonomy_ready: boolean;
  multi_level_case_cause_loader_ready: boolean;
  experience_package_manifest_ready: boolean;
  skill_manifest_loader_ready: boolean;
  case_cause_fallback_ready: boolean;
  skill_context_dry_run_ready: boolean;
  codex_fine_tune_training_disabled: boolean;
  training_artifact_open_case_training_disabled: boolean;
  training_artifact_skill_auto_publish_disabled: boolean;
  closed_case_training_run_ready: boolean;
  synthetic_closed_case_samples_ready: boolean;
  training_run_manifest_ready: boolean;
  generated_experience_packages_ready: boolean;
  generated_skill_manifests_ready: boolean;
  generated_evaluation_gate_test_cases_ready: boolean;
  generated_loading_manifest_ready: boolean;
  training_run_load_dry_run_ready: boolean;
  training_run_open_case_training_disabled: boolean;
  training_run_skill_auto_publish_disabled: boolean;
  training_run_fine_tune_disabled: boolean;
  training_run_real_case_material_read_disabled: boolean;
  real_closed_case_training_intake_ready: boolean;
  real_closed_case_redaction_pipeline_ready: boolean;
  real_closed_case_classification_ready: boolean;
  real_closed_case_training_sample_segmentation_ready: boolean;
  real_closed_case_source_trace_ready: boolean;
  real_closed_case_review_queue_ready: boolean;
  ...
  skill_package_versioning_ready: boolean;
  skill_package_manifest_ready: boolean;
  skill_package_source_trace_ready: boolean;
  skill_package_audit_ready: boolean;
  skill_package_system_validation_gate_ready: boolean;
  skill_package_final_review_not_applicable: boolean;
  skill_package_provider_call_disabled: boolean;
  skill_package_raw_content_blocked: boolean;
  skill_package_api_key_read_disabled: boolean;
  skill_package_training_disabled: boolean;
  skill_package_publish_disabled: boolean;
  internal_training_experience_package_ready: boolean;
  internal_training_task_builder_ready: boolean;
  internal_experience_package_builder_ready: boolean;
  internal_training_pending_practice_review_ready: boolean;
  internal_training_provider_call_disabled: boolean;
  internal_training_api_key_read_disabled: boolean;
  internal_training_real_training_disabled: boolean;
  internal_training_skill_publish_disabled: boolean;
  practice_load_review_gate_ready: boolean;
  lawyer_experience_editor_ready: boolean;
  practice_load_revalidation_ready: boolean;
  practice_load_lawyer_approval_required: boolean;
  practice_load_provider_call_disabled: boolean;
  practice_load_api_key_read_disabled: boolean;
  practice_load_real_training_disabled: boolean;
  practice_load_skill_publish_disabled: boolean;
  practice_runtime_loading_deferred_to_v731g: boolean;
  external_delivery_disabled: boolean;
  public_link_disabled: boolean;
  email_sending_disabled: boolean;
...
export type PersonalProductionConsoleSummary = {
  ...
  v7_23_readiness: Record<string, boolean>;
  v7_25_readiness: Record<string, boolean>;
  v7_26_readiness: Record<string, boolean>;
  v7_27_readiness: Record<string, boolean>;
  v7_28_readiness: Record<string, boolean>;
  v7_29_readiness: Record<string, boolean>;
  v7_30_readiness: Record<string, boolean>;
  v7_31_readiness: Record<string, boolean>;
  v7_31a_readiness: Record<string, boolean>;
  mock_or_redacted_only: boolean;
  raw_content_included: boolean;
  final_legal_opinion_generated: boolean;
  final_report_generated: boolean;
...
export type PersonalMaterialLiveSafetyBase = {
  owner_only: boolean;
  metadata_only: boolean;
  draft_only: boolean;
  dry_run: boolean;
  ...
  raw_content_exposed: boolean;
  raw_ocr_text_exposed: boolean;
  raw_document_content_exposed: boolean;
  ai_prompt_injected: boolean;
  source_trace_required: boolean;
  lawyer_review_required: boolean;
  audit_required: boolean;
  ...
  final_legal_opinion_generated: boolean;
  final_report_generated: boolean;
  real_pdf_generated: boolean;
  real_docx_generated: boolean;
  public_link_created: boolean;
  email_sent: boolean;
  external_delivery_triggered: boolean;
};

export type PersonalMaterialLiveProviderReadiness = PersonalMaterialLiveSafetyBase & {
  provider_id: string;
  display_name: string;
  provider_type: string;
  provider_category: string;
...
export type PersonalMaterialLiveProviderReadinessList = PersonalMaterialLiveSafetyBase & {
  providers: PersonalMaterialLiveProviderReadiness[];
  provider_count: number;
  dry_run_ready_count: number;
  key_loaded_count: number;
  live_disabled_count: number;
...
export type PersonalMaterialLiveSecretBoundary = PersonalMaterialLiveSafetyBase & {
  provider_id: string;
  key_env_names: string[];
  key_required: boolean;
  key_loaded: boolean;
...
export type PersonalMaterialLiveGateMockRequest = {
  provider_id: string;
  explicit_live_confirmation: boolean;
  owner_authorized: boolean;
  raw_content_boundary_acknowledged: boolean;
  no_ai_prompt_injection_acknowledged: boolean;
  audit_acknowledged: boolean;
};

export type PersonalMaterialLiveGateStatus = PersonalMaterialLiveSafetyBase & {
  gate_id: string;
  provider_id: string;
  global_live_enabled: boolean;
  provider_live_enabled: boolean;
...
export type PersonalMaterialLiveGateList = PersonalMaterialLiveSafetyBase & {
  live_gates: PersonalMaterialLiveGateStatus[];
  live_gate_count: number;
  warnings: string[];
};

export type PersonalMaterialLiveHealthDryRun = PersonalMaterialLiveSafetyBase & {
  provider_id: string;
  config_detected: boolean;
  key_loaded: boolean;
  live_gate_status: string;
...
export type LiveConnectionSafetyBase = {
  owner_only: boolean;
  lawyer_gated: boolean;
  metadata_only: boolean;
  draft_only: boolean;
  dry_run: boolean;
  provider_gated: boolean;
  ...
  raw_content_exposed: boolean;
  raw_ocr_text_exposed: boolean;
  raw_provider_response_exposed: boolean;
  local_path_exposed: boolean;
  source_trace_required: boolean;
  lawyer_review_required: boolean;
  audit_required: boolean;
  training_data_generated: boolean;
...
export type LiveConnectionStatus = LiveConnectionSafetyBase & {
  enabled: boolean;
  version: string;
  runtime_label: string;
  provider_count: number;
  ...
  key_loaded_count: number;
  live_disabled_count: number;
  live_connection_ready: boolean;
  quality_metadata_ready: boolean;
  usage_cost_metadata_ready: boolean;
...
export type LiveConnectionProvider = LiveConnectionSafetyBase & {
  provider_id: string;
  display_name: string;
  provider_category: string;
  provider_type: string;
...
export type LiveConnectionProviderList = LiveConnectionSafetyBase & {
  providers: LiveConnectionProvider[];
  provider_count: number;
  category_count: number;
  dry_run_ready_count: number;
  key_loaded_count: number;
...
export type LiveConnectionRuntimeList = LiveConnectionSafetyBase & {
  runtimes: Array<{ runtime_id: string; label: string; category: string; target_route: string; status: string; enabled: boolean; dry_run_ready: boolean; live_enabled: boolean; warnings: string[] }>;
  runtime_count: number;
  warnings: string[];
};

export type LiveConnectionSecretBoundary = LiveConnectionSafetyBase & {
  provider_id: string;
  key_env_name?: string | null;
  key_loaded: boolean;
  key_source: string;
...
export type LiveConnectionLiveGate = LiveConnectionSafetyBase & {
  gate_id: string;
  provider_id: string;
  global_live_enabled: boolean;
  provider_live_enabled: boolean;
  ...
  owner_confirmation_received: boolean;
  lawyer_gate_acknowledged: boolean;
  raw_content_boundary_acknowledged: boolean;
  live_gate_status: string;
  live_blocked_reason: string;
  next_required_confirmation: string;
...
export type LiveConnectionUsagePolicy = LiveConnectionSafetyBase & {
  provider_id: string;
  estimated_token_count: number;
  estimated_page_count: number;
  estimated_query_count: number;
  estimated_call_count: number;
  estimated_cost_available: boolean;
  actual_cost_recorded: boolean;
  billable_call_executed: boolean;
...
export type LiveConnectionHealthDryRun = LiveConnectionSafetyBase & {
  provider_id: string;
  config_detected: boolean;
  key_loaded: boolean;
  adapter_registered: boolean;
...
export type LiveConnectionRunRequest = {
  provider_id: string;
  run_type: string;
  case_id: string;
  material_id?: string | null;
  ...
  lawyer_gate_acknowledged: boolean;
  source_trace_acknowledged: boolean;
  raw_content_boundary_acknowledged: boolean;
  draft_only_acknowledged: boolean;
};

export type LiveConnectionRunRecord = LiveConnectionSafetyBase & {
  run_id: string;
  provider_id: string;
  provider_category: string;
  run_type: string;
  ...
  cost_metadata: Record<string, any>;
...
export type LiveConnectionRunList = LiveConnectionSafetyBase & {
  runs: LiveConnectionRunRecord[];
  run_count: number;
  warnings: string[];
};

export type LiveConnectionAuditTimeline = LiveConnectionSafetyBase & {
  events: Array<LiveConnectionSafetyBase & { event_id: string; provider_id: string; action: string; run_id?: string | null; created_at: string; warnings: string[] }>;
  event_count: number;
  warnings: string[];
};

export type LiveConnectionSafetyStatus = LiveConnectionSafetyBase & {
  safety_items: string[];
  safety_item_count: number;
  all_safety_checks_passed: boolean;
  warnings: string[];
...
export type LegalEnterpriseSafetyBase = {
  metadata_only: boolean;
  dry_run: boolean;
  provider_gated: boolean;
  live_default_enabled: boolean;
...
export type LegalEnterpriseProvider = LegalEnterpriseSafetyBase & {
  provider_id: string;
  provider_name: string;
  provider_category: string;
...
export type LegalEnterpriseStatus = LegalEnterpriseSafetyBase & {
  enabled: boolean;
  version: string;
  runtime_label: string;
  legal_gateway_ready: boolean;
  enterprise_gateway_ready: boolean;
  legal_source_trace_ready: boolean;
  enterprise_verification_ready: boolean;
  provider_count: number;
  legal_provider_count: number;
  enterprise_provider_count: number;
  warnings: string[];
};

export type LegalEnterpriseProviderList = LegalEnterpriseSafetyBase & {
  providers: LegalEnterpriseProvider[];
  provider_count: number;
  key_loaded_count: number;
  dry_run_ready_count: number;
  live_disabled_count: number;
...
export type LegalEnterpriseCategorySummaryList = LegalEnterpriseSafetyBase & {
  categories: Array<LegalEnterpriseSafetyBase & { category: string; provider_count: number; key_loaded_count: number; dry_run_ready_count: number; live_disabled_count: number; blocked_provider_count: number }>;
  category_count: number;
};

export type LegalEnterpriseGenericResponse = LegalEnterpriseSafetyBase & Record<string, any>;
...
export type TrainingArtifactSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  metadata_only: boolean;
  training_artifact_only: boolean;
  codex_training_scheme: boolean;
  fine_tune_model_training: boolean;
  closed_case_only: boolean;
  open_case_data_used: boolean;
  raw_content_included: boolean;
  raw_ocr_content_included: boolean;
  api_key_exposed: boolean;
  ...
  skill_published: boolean;
  skill_auto_published: boolean;
  case_cause_taxonomy_required: boolean;
  multi_level_case_cause_enabled: boolean;
  case_cause_match_required: boolean;
  fallback_supported: boolean;
  load_dry_run: boolean;
  load_executed: boolean;
  final_legal_opinion_generated: boolean;
  final_report_generated: boolean;
  public_link_created: boolean;
  email_sent: boolean;
...
export type TrainingArtifactStatus = TrainingArtifactSafetyBase & {
  version: string;
  status: string;
  training_artifact_loader_ready: boolean;
  codex_training_scheme_ready: boolean;
  case_cause_taxonomy_ready: boolean;
  multi_level_loader_ready: boolean;
  experience_package_manifest_ready: boolean;
  skill_manifest_loader_ready: boolean;
  ...
  package_count: number;
  skill_count: number;
  taxonomy_node_count: number;
  warnings: string[];
};
export type CodexTrainingScheme = TrainingArtifactSafetyBase & { scheme_id: string; scheme_version: string; display_name_cn: string; target_skill_ids: string[]; training_steps: string[]; output_artifacts: string[] };
export type CaseCauseNode = TrainingArtifactSafetyBase & { case_cause_id: string; case_domain: string; case_cause_code: string; case_cause_name: string; level: number; case_cause_path: string[]; parent_id?: string | null; child_ids: string[] };
export type CaseCauseTaxonomyManifest = TrainingArtifactSafetyBase & { taxonomy_id: string; taxonomy_version: string; nodes: CaseCauseNode[]; node_count: number; supported_levels: number[] };
export type TrainingArtifactManifestList = TrainingArtifactSafetyBase & { artifacts: Array<Record<string, unknown>>; artifact_count: number; warnings: string[] };
export type TrainingArtifactCaseCauseMatchRequest = {
  case_domain: string;
  case_cause_level_1: string;
  case_cause_level_2: string;
...
export type TrainingArtifactLoadDryRunRequest = TrainingArtifactCaseCauseMatchRequest & {
  target_skill_ids: string[];
  explicit_dry_run_confirmation: boolean;
  explicit_no_training_confirmation: boolean;
  explicit_no_open_case_training_confirmation: boolean;
  explicit_no_auto_publish_confirmation: boolean;
};
export type TrainingArtifactCaseCauseMatchResult = TrainingArtifactSafetyBase & {
  match_id: string;
  requested_case_cause_path: string[];
  matched_case_cause_id?: string | null;
  exact_package_ids: string[];
  ancestor_fallback_package_ids: string[];
  common_package_ids: string[];
  evidence_overlay_package_ids: string[];
  selected_package_ids: string[];
  fallback_chain: string[];
  merge_order: string[];
  warnings: string[];
};
export type TrainingArtifactSkillContext = TrainingArtifactSafetyBase & { skill_context_id: string; source_match_id: string; selected_skill_ids: string[]; selected_package_ids: string[]; case_cause_path: string[]; fallback_chain: string[]; optimization_suggestions: string[] };
export type TrainingArtifactLoadDryRun = TrainingArtifactSafetyBase & { run_id: string; run_status: string; match_result: TrainingArtifactCaseCauseMatchResult; skill_context: TrainingArtifactSkillContext; warnings: string[] };
export type TrainingArtifactLoadDryRunList = TrainingArtifactSafetyBase & { load_dry_runs: TrainingArtifactLoadDryRun[]; run_count: number };
export type TrainingArtifactSkillContextList = TrainingArtifactSafetyBase & { skill_contexts: TrainingArtifactSkillContext[]; skill_context_count: number };
export type TrainingArtifactSafetyStatus = TrainingArtifactSafetyBase & { safety_checklist: string[]; safety: Record<string, boolean>; all_safety_checks_passed: boolean; warnings: string[] };
export type ExperienceLifecycleSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  metadata_only: boolean;
  lifecycle_view_only: boolean;
  redacted_abstracted_experience_only: boolean;
  lawyer_approval_required_for_runtime_load: boolean;
  source_trace_required: boolean;
  audit_required: boolean;
  feedback_does_not_mutate_loaded_package: boolean;
  candidate_pack_does_not_mutate_loaded_package: boolean;
  next_package_requires_load_review: boolean;
  provider_call_executed: boolean;
  key_value_read: boolean;
  credential_value_returned: boolean;
  provider_result_payload_returned: boolean;
  source_content_returned: boolean;
  source_material_returned: boolean;
  full_output_returned: boolean;
  case_material_returned: boolean;
  loaded_package_mutated: boolean;
  next_package_draft_auto_loaded: boolean;
  automatic_training_triggered: boolean;
  skill_updated: boolean;
  skill_published: boolean;
  final_legal_opinion_generated: boolean;
  final_report_generated: boolean;
...
export type ExperienceLifecycleStageEvent = ExperienceLifecycleSafetyBase & {
  ...
  previous_stage_event_id?: string | null;
  next_stage_candidates: string[];
...
export type ExperienceLifecycleRecord = ExperienceLifecycleSafetyBase & {
  lifecycle_id: string;
  current_stage: string;
  ...
  skill_draft_ids: string[];
  skill_package_ids: string[];
  training_task_ids: string[];
  experience_package_ids: string[];
  practice_load_review_ids: string[];
  runtime_load_ids: string[];
  usage_event_ids: string[];
  observation_ids: string[];
  feedback_ids: string[];
  risk_event_ids: string[];
  candidate_pack_ids: string[];
  next_package_ids: string[];
  latest_loaded_package_id?: string | null;
  latest_lawyer_approved_package_id?: string | null;
  latest_next_package_draft_id?: string | null;
  blocked_reason?: string | null;
  stage_events: ExperienceLifecycleStageEvent[];
  next_allowed_actions: string[];
  created_at: string;
  updated_at: string;
  warnings: string[];
};
export type ExperienceLifecycleList = ExperienceLifecycleSafetyBase & { lifecycles: ExperienceLifecycleRecord[]; lifecycle_count: number; warnings: string[] };
export type ExperienceLifecycleStatus = ExperienceLifecycleSafetyBase & { version: string; status: string; lifecycle_registry_ready: boolean; stage_event_count: number; lifecycle_count: number; warnings: string[] };
export type ExperienceLifecycleGraphNode = ExperienceLifecycleSafetyBase & { id: string; type: string; label: string; status: string; version?: string | null; created_at?: string | null; safety_status: string };
export type ExperienceLifecycleGraphEdge = { from_id: string; to_id: string; relation: string; created_at?: string | null };
export type ExperiencePackageLineage = ExperienceLifecycleSafetyBase & { lineage_id: string; root_package_id: string; package_id: string; package_version: string; package_kind: string; lineage_status: string; created_at: string };
export type ExperienceLifecycleGraph = ExperienceLifecycleSafetyBase & { lifecycle_id: string; nodes: ExperienceLifecycleGraphNode[]; edges: ExperienceLifecycleGraphEdge[]; lineage: ExperiencePackageLineage[]; warnings: string[] };
export type ExperienceLifecycleAuditTimeline = ExperienceLifecycleSafetyBase & { audit_timeline_id: string; lifecycle_id: string; events_count: number; latest_event_at?: string | null; risk_events_count: number; blocked_events_count: number; manual_load_review_events_count: number; runtime_events_count: number; feedback_events_count: number; events: Array<Record<string, unknown>>; warnings: string[] };
export type ExperienceLifecycleSourceTraceView = ExperienceLifecycleSafetyBase & { source_trace_view_id: string; lifecycle_id: string; root_source_trace_id: string; missing_trace_warnings: string[]; created_at: string; [key: string]: unknown };
export type ExperienceLifecycleIntegrityCheck = ExperienceLifecycleSafetyBase & { integrity_check_id: string; lifecycle_id: string; status: string; passed_checks: string[]; failed_checks: string[]; recommended_actions: string[]; warnings: string[]; created_at: string };
export type ExperienceLifecycleSafetySummary = ExperienceLifecycleSafetyBase & { lifecycle_id: string; raw_content_absent: boolean; ocr_payload_absent: boolean; filesystem_location_absent: boolean; credential_value_absent: boolean; provider_payload_absent: boolean; loaded_package_is_lawyer_approved: boolean; overall_safety_status: string; warnings: string[] };
export type V732ExperienceLifecycleStatus = ExperienceLifecycleStatus;
export type CaseAnalysisWorkbenchSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  metadata_only: boolean;
  schema_driven_output_only: boolean;
  redacted_abstracted_output_only: boolean;
  frontend_output_definition_forbidden: boolean;
  lawyer_review_required: boolean;
  source_trace_required: boolean;
  audit_required: boolean;
  provider_call_executed: boolean;
  key_value_read: boolean;
  credential_value_returned: boolean;
  provider_payload_returned: boolean;
  source_content_returned: boolean;
  case_material_returned: boolean;
  loaded_package_mutated: boolean;
  next_package_auto_generated: boolean;
...
export type CaseAnalysisRuntimeOutput = CaseAnalysisWorkbenchSafetyBase & {
  output_id: string;
  group_id: string;
  output_type: string;
  output_title: string;
  ...
  output_summary_redacted: string;
  output_detail_redacted: string;
  risk_level: string;
  confidence_label: string;
  source_experience_ids: string[];
  source_runtime_load_id: string;
  source_usage_event_id?: string | null;
  source_trace_id: string;
  audit_id: string;
...
export type CaseAnalysisOutputGroup = CaseAnalysisWorkbenchSafetyBase & {
  group_id: string;
  group_title: string;
  group_type: string;
  ...
  description: string;
  outputs: CaseAnalysisRuntimeOutput[];
};
export type CaseAnalysisSkillOutputSchema = CaseAnalysisWorkbenchSafetyBase & {
  skill_id: string;
  skill_name: string;
  ...
  runtime_load_id: string;
  output_groups: CaseAnalysisOutputGroup[];
  warnings: string[];
};
export type CaseAnalysisSummaryMetrics = CaseAnalysisWorkbenchSafetyBase & {
  total_outputs: number;
  fact_output_count: number;
  legal_analysis_output_count: number;
  risk_flagged_count: number;
  feedback_count: number;
  high_risk_count: number;
  reviewed_count: number;
};
export type CaseAnalysisWorkbenchView = CaseAnalysisWorkbenchSafetyBase & {
  view_id: string;
  case_id: string;
  case_cause_name: string;
  package_id: string;
  package_version: string;
  runtime_load_id: string;
  runtime_load_status: string;
  skill_id: string;
  skill_name: string;
  skill_version: string;
  output_groups: CaseAnalysisOutputGroup[];
  summary_metrics: CaseAnalysisSummaryMetrics;
  safety_summary: Record<string, boolean | string>;
  audit_id: string;
  source_trace_id: string;
  created_at: string;
  updated_at: string;
...
export type CaseAnalysisWorkbenchViewList = CaseAnalysisWorkbenchSafetyBase & { views: CaseAnalysisWorkbenchView[]; view_count: number; warnings: string[] };
export type CaseAnalysisRuntimeOutputList = CaseAnalysisWorkbenchSafetyBase & { outputs: CaseAnalysisRuntimeOutput[]; output_count: number; warnings: string[] };
export type CaseAnalysisOutputFeedback = CaseAnalysisWorkbenchSafetyBase & { feedback_id: string; output_id: string; feedback_type: string; feedback_summary: string; severity: string; created_at: string; warnings: string[] };
export type CaseAnalysisOutputRiskEvent = CaseAnalysisWorkbenchSafetyBase & { risk_event_id: string; output_id: string; risk_level: string; risk_summary: string; mitigation_note: string; created_at: string; warnings: string[] };
export type CaseAnalysisOutputFeedbackRequest = { reviewer_id?: string; feedback_type: string; feedback_summary: string; severity: string; explicit_metadata_only_confirmation: boolean; explicit_no_training_confirmation: boolean };
export type CaseAnalysisOutputRiskEventRequest = { reporter_id?: string; risk_level: string; risk_summary: string; mitigation_note: string; explicit_metadata_only_confirmation: boolean; explicit_no_external_delivery_confirmation: boolean };
export type CaseAnalysisOutputFeedbackList = CaseAnalysisWorkbenchSafetyBase & { output_id: string; feedback: CaseAnalysisOutputFeedback[]; feedback_count: number; warnings: string[] };
export type CaseAnalysisOutputRiskEventList = CaseAnalysisWorkbenchSafetyBase & { output_id: string; risk_events: CaseAnalysisOutputRiskEvent[]; risk_event_count: number; warnings: string[] };
export type CaseAnalysisOutputAudit = CaseAnalysisWorkbenchSafetyBase & { output_id: string; audit_id: string; events: Array<Record<string, string>>; event_count: number; warnings: string[] };
export type CaseAnalysisOutputSourceTrace = CaseAnalysisWorkbenchSafetyBase & { output_id: string; source_trace_id: string; source_experience_ids: string[]; source_runtime_load_id: string; source_usage_event_id?: string | null; trace_status: string; trace_summary: string[]; warnings: string[] };
export type V733CaseAnalysisWorkbenchStatus = CaseAnalysisWorkbenchSafetyBase & { version: string; status: string; view_count: number; output_count: number; fact_group_count: number; legal_analysis_group_count: number; feedback_count: number; risk_event_count: number; warnings: string[] };
export type CaseAnalysisImprovementSafetyBase = CaseAnalysisWorkbenchSafetyBase & {
  improvement_candidate_only: boolean;
  redacted_abstracted_metadata_only: boolean;
  output_feedback_input_only: boolean;
  risk_event_input_only: boolean;
  loaded_package_auto_mutated: boolean;
  lawyer_approved_package_auto_mutated: boolean;
  output_schema_auto_mutated: boolean;
  runtime_package_auto_replaced: boolean;
  training_dataset_auto_built: boolean;
  training_gate_required: boolean;
  package_disable_auto_executed: boolean;
  package_rollback_auto_executed: boolean;
};
export type CaseAnalysisImprovementCandidateType = string;
export type CaseAnalysisProposedChangeType = string;
export type CaseAnalysisImprovementCandidate = CaseAnalysisImprovementSafetyBase & {
  candidate_id: string;
  source_output_id: string;
  source_output_group: string;
  source_output_type: string;
  source_output_title: string;
  source_output_order: number;
  source_output_summary_redacted: string;
  source_case_analysis_view_id: string;
  source_runtime_load_id: string;
  source_usage_event_id?: string | null;
  source_package_id: string;
  source_package_version: string;
  source_experience_card_ids: string[];
  source_feedback_ids: string[];
  source_risk_event_ids: string[];
  source_audit_ids: string[];
  source_trace_ids: string[];
  source_feedback_types: string[];
  source_risk_types: string[];
  candidate_type: CaseAnalysisImprovementCandidateType;
  candidate_status: string;
  candidate_severity: string;
  candidate_title: string;
  candidate_summary: string;
  candidate_reason: string;
  proposed_change_type: CaseAnalysisProposedChangeType;
  proposed_change_summary: string;
  target_object_type: string;
  target_object_id: string;
  affected_output_schema_group: string;
  affected_output_schema_type: string;
  affected_usage_boundary: string;
  affected_risk_warning: string;
  affected_experience_card_id?: string | null;
...
export type CaseAnalysisImprovementCandidateList = CaseAnalysisImprovementSafetyBase & { candidates: CaseAnalysisImprovementCandidate[]; candidate_count: number; warnings: string[] };
export type CaseAnalysisImprovementReadinessReport = CaseAnalysisImprovementSafetyBase & { readiness_report_id: string; candidate_id: string; status: string; passed_checks: string[]; failed_checks: string[]; warnings: string[]; blocked_reason?: string | null; recommended_next_action: string; created_at: string; audit_id: string };
export type CaseAnalysisOutputToExperienceTrace = CaseAnalysisImprovementSafetyBase & { trace_id: string; output_id: string; output_group: string; output_type: string; runtime_load_id: string; package_id: string; package_version: string; experience_package_id: string; experience_card_ids: string[]; skill_schema_id: string; skill_schema_version: string; usage_event_id?: string | null; audit_id: string; source_trace_id: string; trace_status: string; missing_trace_warnings: string[]; created_at: string; warnings: string[] };
export type CaseAnalysisOutputToExperienceTraceList = CaseAnalysisImprovementSafetyBase & { output_traces: CaseAnalysisOutputToExperienceTrace[]; trace_count: number; warnings: string[] };
export type CaseAnalysisImprovementDiff = CaseAnalysisImprovementSafetyBase & { diff_id: string; candidate_ids: string[]; source_package_id: string; source_package_version: string; target_next_package_version_hint: string; added_cards_count: number; revised_cards_count: number; deleted_cards_count: number; boundary_changes_count: number; risk_warning_changes_count: number; schema_metadata_changes_count: number; training_update_recommendations_count: number; disable_recommendations_count: number; rollback_recommendations_count: number; diff_summary: string; risk_summary: string; readiness_status: string; audit_id: string; source_trace_id: string; created_at: string; warnings: string[] };
export type CaseAnalysisImprovementDiffList = CaseAnalysisImprovementSafetyBase & { diffs: CaseAnalysisImprovementDiff[]; diff_count: number; warnings: string[] };
export type CaseAnalysisImprovementSummary = CaseAnalysisImprovementCandidateList;
export type CaseAnalysisImprovementStatus = CaseAnalysisImprovementSafetyBase & { version: string; status: string; candidate_count: number; ready_for_training_dataset_build_count: number; trace_count: number; diff_count: number; feedback_count: number; risk_event_count: number; warnings: string[] };
export type V734CaseAnalysisImprovementStatus = CaseAnalysisImprovementStatus;
export type TrainingDatasetSafetyBase = CaseAnalysisImprovementSafetyBase & {
  training_dataset_manifest_generated: boolean;
  training_examples_generated: boolean;
  training_task_plan_generated: boolean;
  training_gate_report_generated: boolean;
  gate_reference_only: boolean;
  quality_reference_only: boolean;
  blocks_next_stage: boolean;
  ready_candidate_only: boolean;
  candidate_audit_checked: boolean;
  source_trace_checked: boolean;
  sensitive_metadata_scan_required: boolean;
  sensitive_metadata_scan_passed: boolean;
  experience_package_loaded_as_metadata: boolean;
  skill_output_schema_loaded_as_metadata: boolean;
  output_to_experience_trace_loaded_as_metadata: boolean;
  formal_training_set_written: boolean;
  real_training_triggered: boolean;
  real_training_output_generated: boolean;
  skill_updated: boolean;
};
export type TrainingDatasetExample = TrainingDatasetSafetyBase & {
  example_id: string;
  candidate_id: string;
  source_output_id: string;
  source_output_group: string;
  ...
  output_trace_status: string;
  skill_schema_id: string;
  skill_schema_version: string;
  training_input_summary: string;
  training_target_summary: string;
  training_task_type: string;
  gate_input_status: string;
  metadata_safety_status: string;
  created_at: string;
  warnings: string[];
};
export type TrainingTaskPlanMetadata = TrainingDatasetSafetyBase & { task_plan_id: string; dataset_id: string; task_plan_status: string; target_skill_ids: string[]; source_package_ids: string[]; source_candidate_ids: string[]; example_count: number; planned_steps: string[]; blocked_actions: string[]; created_at: string; warnings: string[] };
export type TrainingDatasetManifest = TrainingDatasetSafetyBase & { dataset_id: string; dataset_version: string; dataset_status: string; source_candidate_ids: string[]; source_package_ids: string[]; source_skill_ids: string[]; source_output_ids: string[]; source_trace_ids: string[]; source_audit_ids: string[]; candidate_count: number; example_count: number; examples: TrainingDatasetExample[]; task_plan: TrainingTaskPlanMetadata; audit_id: string; source_trace_id: string; created_at: string; updated_at: string; warnings: string[] };
export type TrainingDatasetExampleList = TrainingDatasetSafetyBase & { dataset_id?: string | null; examples: TrainingDatasetExample[]; example_count: number; warnings: string[] };
export type TrainingGateReport = TrainingDatasetSafetyBase & { gate_report_id: string; dataset_id?: string | null; gate_status: string; candidate_count: number; example_count: number; passed_checks: string[]; failed_checks: string[]; gate_summary: string; candidate_metadata_safe: boolean; audit_source_trace_safe: boolean; provider_boundary_safe: boolean; package_mutation_safe: boolean; training_boundary_safe: boolean; recommended_next_action: string; created_at: string; audit_id: string; source_trace_id: string; warnings: string[] };
export type TrainingDatasetStatus = TrainingDatasetSafetyBase & { version: string; status: string; dataset_builder_ready: boolean; training_gate_ready: boolean; ready_candidate_count: number; dataset_manifest_count: number; latest_dataset_id?: string | null; latest_gate_status: string; example_count: number; warnings: string[] };
export type CodexTrainingDryRunSafetyBase = TrainingDatasetSafetyBase & {
  codex_skill_dry_run: boolean;
  internal_training_simulation_only: boolean;
  provider_access_attempted: boolean;
  runtime_package_written: boolean;
  runtime_package_replaced: boolean;
};
export type CodexTrainingDryRunLogEntry = CodexTrainingDryRunSafetyBase & { log_id: string; run_id: string; step_name: string; step_status: string; message: string; candidate_ids: string[]; dataset_id?: string | null; gate_report_id?: string | null; loaded_metadata_refs: Array<Record<string, string>>; created_at: string; warnings: string[] };
export type CodexTrainingDryRun = CodexTrainingDryRunSafetyBase & { run_id: string; run_status: string; dataset_id?: string | null; gate_report_id?: string | null; candidate_ids: string[]; example_count: number; training_gate_status: string; loaded_metadata_refs: Array<Record<string, string>>; logs: CodexTrainingDryRunLogEntry[]; audit_id: string; source_trace_id: string; created_at: string; warnings: string[] };
export type CodexTrainingDryRunLogList = CodexTrainingDryRunSafetyBase & { logs: CodexTrainingDryRunLogEntry[]; log_count: number; latest_run_id?: string | null; warnings: string[] };
export type CodexTrainingDryRunGateReport = CodexTrainingDryRunSafetyBase & { gate_report_id: string; run_id?: string | null; dataset_gate_report_id?: string | null; gate_status: string; passed_checks: string[]; failed_checks: string[]; gate_summary: string; provider_boundary_safe: boolean; key_boundary_safe: boolean; runtime_package_boundary_safe: boolean; training_boundary_safe: boolean; publication_boundary_safe: boolean; created_at: string; audit_id: string; source_trace_id: string; warnings: string[] };
export type CodexTrainingDryRunStatus = CodexTrainingDryRunSafetyBase & { version: string; status: string; dryrun_engine_ready: boolean; dryrun_count: number; latest_run_id?: string | null; latest_run_status: string; latest_gate_status: string; log_count: number; warnings: string[] };
export type CodexInternalTrainingSafetyBase = CodexTrainingDryRunSafetyBase & {
  codex_skill_internal_training_run: boolean;
  internal_training_workspace_only: boolean;
  local_cpu_gpu_mode_allowed: boolean;
  internal_model_metadata_generated: boolean;
  training_metrics_generated: boolean;
  dryrun_log_compared: boolean;
  external_provider_training_triggered: boolean;
  real_training_output_exported: boolean;
};
export type CodexInternalTrainingMetrics = CodexInternalTrainingSafetyBase & { metrics_id: string; run_id: string; dataset_id?: string | null; dryrun_id?: string | null; candidate_count: number; example_count: number; log_alignment_score: number; metadata_safety_score: number; gate_pass_rate: number; provider_call_count: number; key_read_count: number; runtime_package_mutation_count: number; skill_publish_count: number; internal_model_artifact_id: string; internal_model_artifact_status: string; warnings: string[] };
export type CodexInternalTrainingLogEntry = CodexInternalTrainingSafetyBase & { log_id: string; run_id: string; step_name: string; step_status: string; message: string; dryrun_log_ref?: string | null; dataset_id?: string | null; candidate_ids: string[]; created_at: string; warnings: string[] };
export type CodexInternalTrainingRun = CodexInternalTrainingSafetyBase & { run_id: string; run_status: string; execution_mode: string; dataset_id?: string | null; dryrun_id?: string | null; gate_report_id?: string | null; candidate_ids: string[]; example_count: number; metrics: CodexInternalTrainingMetrics; logs: CodexInternalTrainingLogEntry[]; audit_id: string; source_trace_id: string; created_at: string; warnings: string[] };
export type CodexInternalTrainingLogList = CodexInternalTrainingSafetyBase & { logs: CodexInternalTrainingLogEntry[]; log_count: number; latest_run_id?: string | null; warnings: string[] };
export type CodexInternalTrainingGateReport = CodexInternalTrainingSafetyBase & { gate_report_id: string; run_id?: string | null; gate_status: string; passed_checks: string[]; failed_checks: string[]; metrics_summary: Record<string, number | string>; gate_summary: string; dryrun_log_comparison_status: string; provider_boundary_safe: boolean; key_boundary_safe: boolean; runtime_package_boundary_safe: boolean; publication_boundary_safe: boolean; audit_source_trace_safe: boolean; created_at: string; audit_id: string; source_trace_id: string; warnings: string[] };
export type CodexInternalTrainingStatus = CodexInternalTrainingSafetyBase & { version: string; status: string; training_run_engine_ready: boolean; training_run_count: number; latest_run_id?: string | null; latest_run_status: string; latest_gate_status: string; latest_metrics_id?: string | null; log_count: number; warnings: string[] };
export type RawTrainingMaterialSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  metadata_only: boolean;
  local_private_processing_only: boolean;
  controlled_source_zone_only: boolean;
  source_input_in_controlled_zone: boolean;
  frontend_source_display_allowed: boolean;
  api_source_payload_allowed: boolean;
  git_tracking_allowed: boolean;
  ...
  source_content_returned: boolean;
  case_material_returned: boolean;
  filesystem_location_exposed: boolean;
  redacted_output_only: boolean;
  abstracted_output_only: boolean;
  source_trace_required: boolean;
  audit_required: boolean;
  training_triggered: boolean;
  skill_published: boolean;
  runtime_package_replaced: boolean;
  practice_load_review_required: boolean;
  final_legal_opinion_generated: boolean;
  final_report_generated: boolean;
  public_link_created: boolean;
  email_sent: boolean;
...
export type RawTrainingMaterialBoundaryStatus = RawTrainingMaterialSafetyBase & { boundary_id: string; owner_user_id: string; workspace_id: string; material_batch_id: string; controlled_zone_status: string; allowed_material_types: string[]; controlled_read_allowed: boolean; warnings: string[] };
export type TrainingMaterial = RawTrainingMaterialSafetyBase & { training_material_id: string; material_batch_id: string; material_kind: string; material_label: string; material_status: string; source_zone_status: string; ocr_status: string; document_parse_status: string; structure_status: string; legal_retrieval_status: string; parse_quality_status: string; redaction_output_status: string; source_trace_id: string; audit_id: string; warnings: string[] };
export type TrainingMaterialList = RawTrainingMaterialSafetyBase & { training_materials: TrainingMaterial[]; material_count: number; warnings: string[] };
export type OCRJob = RawTrainingMaterialSafetyBase & { ocr_job_id: string; training_material_id: string; material_kind: string; ocr_status: string; ocr_engine_mode: string; controlled_internal_ref: string; redacted_preview_available: boolean; quality_score: number; source_trace_id: string; audit_id: string; created_at: string; warnings: string[] };
export type DocumentParseJob = RawTrainingMaterialSafetyBase & { parse_job_id: string; training_material_id: string; parse_status: string; document_type: string; sections_detected: string[]; tables_detected: number; entities_detected_count: number; controlled_structured_ref: string; redacted_summary: string; quality_score: number; source_trace_id: string; audit_id: string; created_at: string; warnings: string[] };
export type JudgmentStructure = RawTrainingMaterialSafetyBase & { judgment_structure_id: string; training_material_id: string; court_level: string; case_cause: string; parties_redacted: boolean; claims_summary_redacted: string; issues_summary_redacted: string; legal_basis_summary: string; source_trace_id: string; audit_id: string; quality_score: number };
export type LawyerWorkProductStructure = RawTrainingMaterialSafetyBase & { work_product_structure_id: string; training_material_id: string; fact_notes_summary_redacted: string; evidence_strategy_summary_redacted: string; argument_strategy_summary_redacted: string; risk_assessment_summary_redacted: string; source_trace_id: string; audit_id: string; quality_score: number };
export type EvidenceIndex = RawTrainingMaterialSafetyBase & { evidence_index_id: string; material_batch_id: string; evidence_items_count: number; evidence_groups: string[]; proof_purpose_summary: string; evidence_chain_summary_redacted: string; missing_evidence_warnings: string[]; source_trace_id: string; audit_id: string; quality_score: number };
export type TrainingMaterialLegalRetrievalJob = RawTrainingMaterialSafetyBase & { legal_retrieval_job_id: string; material_batch_id: string; case_cause: string; claims_summary_redacted: string; issues_summary_redacted: string; retrieval_status: string; statutes: string[]; judicial_interpretations: string[]; case_law_references: string[]; rule_summaries: string[]; retrieval_source_trace_id: string; audit_id: string; quality_score: number };
export type RuleAlignment = RawTrainingMaterialSafetyBase & { alignment_id: string; material_batch_id: string; fact_to_evidence_links: string[]; issue_to_rule_links: string[]; alignment_summary_redacted: string; missing_alignment_warnings: string[]; quality_score: number; source_trace_id: string; audit_id: string };
export type ParseQualityGateReport = RawTrainingMaterialSafetyBase & { parse_quality_gate_id: string; material_batch_id: string; ocr_completed: boolean; document_parse_completed: boolean; judgment_structured: boolean; lawyer_work_product_structured: boolean; evidence_index_built: boolean; legal_retrieval_completed: boolean; rule_alignment_completed: boolean; source_trace_complete: boolean; audit_complete: boolean; source_payload_not_exported: boolean; quality_score: number; gate_status: string; blocked_reason?: string | null; warnings: string[] };
export type RawBasedExperienceCandidate = RawTrainingMaterialSafetyBase & { raw_based_candidate_id: string; material_batch_id: string; case_cause: string; candidate_type: string; candidate_title: string; candidate_summary_redacted: string; abstraction_level: string; redaction_status: string; source_trace_id: string; audit_id: string; quality_score: number; created_at: string };
export type RedactedExperiencePackage = RawTrainingMaterialSafetyBase & { redacted_experience_package_id: string; material_batch_id: string; case_cause: string; experience_cards: Array<Record<string, unknown>>; source_candidate_ids: string[]; parse_quality_gate_id: string; legal_retrieval_job_ids: string[]; rule_alignment_ids: string[]; redaction_report_id: string; safety_report_id: string; source_trace_bundle_id: string; audit_bundle_id: string; package_status: string; created_at: string; warnings: string[] };
export type V735aStatus = RawTrainingMaterialSafetyBase & { version: string; status: string; boundary_ready: boolean; material_count: number; ocr_job_count: number; parse_job_count: number; legal_retrieval_job_count: number; latest_gate_status: string; warnings: string[] };
export type V735bStatus = RawTrainingMaterialSafetyBase & { version: string; status: string; candidate_count: number; redacted_package_count: number; ready_for_training_dataset_count: number; warnings: string[] };
export type ProviderCredentialContract = { provider_category: string; credential_alias: string; loaded_state_field: string; value_stored_in_skill: boolean };
export type CodexTrainingSkillSpec = RawTrainingMaterialSafetyBase & { training_skill_id: string; training_dataset_id?: string | null; skill_target: string; target_skill_ids: string[]; baseline_complete: boolean; source_skill_ids: string[]; source_package_ids: string[]; derived_from: string[]; source_baseline_refs: Array<Record<string, unknown>>; skill_name: string; skill_version: string; skill_status: string; training_objectives: string[]; input_schema: Record<string, unknown>; output_schema: Record<string, unknown>; output_groups: Array<Record<string, unknown>>; prompt_strategy: Record<string, unknown>; experience_package_refs: string[]; training_example_refs: string[]; evaluation_plan: string[]; safety_constraints: string[]; source_trace_bundle_id: string; audit_bundle_id: string; created_at: string; warnings: string[] };
export type CodexTrainingSkillGateReport = RawTrainingMaterialSafetyBase & { training_skill_gate_report_id: string; training_skill_id: string; gate_status: string; training_dataset_gate_passed: boolean; training_examples_available: boolean; skill_target_defined: boolean; output_schema_aligned: boolean; source_trace_complete: boolean; audit_complete: boolean; not_publishable_by_default: boolean; not_runtime_loadable_by_default: boolean; warnings: string[] };
export type CodexTrainingSkillInterfaceDoc = RawTrainingMaterialSafetyBase & { interface_doc_id: string; title: string; version: string; baseline_skill_ids: string[]; legacy_package_refs: string[]; provider_credential_contract: ProviderCredentialContract[]; request_contracts: Array<Record<string, unknown>>; required_inputs: string[]; forbidden_outputs: string[]; warnings: string[] };
export type CodexSkillTrainingMetrics = RawTrainingMaterialSafetyBase & { metrics_id: string; training_run_id: string; examples_count: number; passed_examples_count: number; failed_examples_count: number; schema_alignment_score: number; safety_score: number; traceability_score: number; readiness_score: number; warnings: string[] };
export type CodexSkillTrainingArtifact = RawTrainingMaterialSafetyBase & { trained_artifact_id: string; training_run_id: string; training_skill_id: string; artifact_status: string; artifact_kind: string; artifact_summary: string; not_publishable: boolean; not_runtime_loadable: boolean; requires_practice_load_review: boolean; source_trace_id: string; audit_id: string; created_at: string };
export type CodexSkillTrainingRun = RawTrainingMaterialSafetyBase & { training_run_id: string; training_skill_id: string; training_dataset_id?: string | null; run_mode: string; run_status: string; started_at: string; finished_at: string; metrics_id: string; gate_report_id: string; artifact_id: string; audit_id: string; source_trace_id: string; logs: Array<Record<string, unknown>>; metrics: CodexSkillTrainingMetrics; artifact: CodexSkillTrainingArtifact; blocked_reason?: string | null; warnings: string[] };
export type V737Status = RawTrainingMaterialSafetyBase & { version: string; status: string; training_skill_count: number; ready_for_dry_run_training_count: number; warnings: string[] };
export type V738Status = RawTrainingMaterialSafetyBase & { version: string; status: string; training_run_count: number; completed_run_count: number; warnings: string[] };
export type CodexTrainingRunSafetyBase = TrainingArtifactSafetyBase & {
  codex_training: boolean;
  training_run_generated: boolean;
  redaction_completed: boolean;
  quality_reference_only: boolean;
};
export type CodexTrainingRunRequest = {
  source_case_mode: string;
  target_case_cause_paths: string[][];
  target_skill_ids: string[];
  explicit_closed_case_only_confirmation: boolean;
  explicit_redaction_confirmation: boolean;
  explicit_no_raw_content_confirmation: boolean;
  explicit_no_open_case_training_confirmation: boolean;
  explicit_no_auto_publish_confirmation: boolean;
};
export type CodexTrainingRun = CodexTrainingRunSafetyBase & Record<string, unknown> & {
  training_run_id: string;
  run_status: string;
  manifest: Record<string, unknown>;
  training_samples: Array<Record<string, unknown>>;
  experience_packages: Array<Record<string, unknown>>;
  generated_skills: Array<Record<string, unknown>>;
  evaluations: Array<Record<string, unknown>>;
  gates: Array<Record<string, unknown>>;
  test_cases: Array<Record<string, unknown>>;
  loading_manifest: Record<string, unknown>;
  warnings: string[];
};
export type CodexTrainingRunList = CodexTrainingRunSafetyBase & { training_runs: CodexTrainingRun[]; run_count: number; warnings: string[] };
export type CodexTrainingRunLoadDryRunResult = CodexTrainingRunSafetyBase & { training_run_id: string; load_dry_run_result: TrainingArtifactLoadDryRun; warnings: string[] };
export type RealClosedCaseIntakeSafetyBase = TrainingArtifactSafetyBase & {
  real_closed_case_intake: boolean;
  redaction_required: boolean;
  redaction_completed: boolean;
  source_trace_required: boolean;
  manual_review_required: boolean;
  ready_for_codex_training: boolean;
};
export type RealClosedCaseTrainingIntakeRequest = {
  case_reference_label: string;
  owner_user_id: string;
  authorization_confirmed: boolean;
  case_closed_confirmed: boolean;
  target_case_cause_path: string[];
  target_skill_ids: string[];
  explicit_no_raw_content_confirmation: boolean;
  explicit_no_open_case_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
};
export type RealClosedCaseTrainingIntake = RealClosedCaseIntakeSafetyBase & {
  intake_id: string;
  source_case_mode: string;
  case_reference_label: string;
  owner_user_id: string;
...
export type RedactionReport = RealClosedCaseIntakeSafetyBase & Record<string, unknown> & { redaction_report_id: string; intake_id: string };
export type CaseCauseClassification = RealClosedCaseIntakeSafetyBase & Record<string, unknown> & { classification_id: string; intake_id: string; case_cause_path: string[] };
export type TrainingSampleSegment = RealClosedCaseIntakeSafetyBase & Record<string, unknown> & { segment_id: string; intake_id: string; segment_type: string; target_skill_id: string };
export type RealClosedCaseTrainingIntakeRecord = RealClosedCaseIntakeSafetyBase & Record<string, unknown> & {
  intake: RealClosedCaseTrainingIntake;
  redaction_report?: RedactionReport | null;
  classification?: CaseCauseClassification | null;
  segments: TrainingSampleSegment[];
  review_queue: Array<Record<string, unknown>>;
  source_traces: Array<Record<string, unknown>>;
  warnings: string[];
};
...
export type SkillPackageStatus =
  ...
  | "system_validated"
  | "validation_failed"
  | "ready_for_training_package_build";
export type V731dSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  local_private_processing_only: boolean;
  approved_experience_only: boolean;
  redacted_output_only: boolean;
  abstracted_experience_only: boolean;
  system_validation_required: boolean;
  system_validation_only: boolean;
  ...
  source_content_returned: boolean;
  source_material_returned: boolean;
  unreviewed_experience_packaged: boolean;
  unsafe_experience_packaged: boolean;
  real_codex_training_triggered: boolean;
  skill_published: boolean;
  skill_publishable: boolean;
  external_delivery_triggered: boolean;
};
export type SkillPackageBuildRequest = {
  source_draft_id?: string | null;
  package_name: string;
  package_version: string;
  supersedes_package_id?: string | null;
  explicit_system_validation_gate_confirmation: boolean;
  explicit_no_manual_training_output_review_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
  explicit_no_real_training_confirmation: boolean;
  explicit_no_skill_publish_confirmation: boolean;
};
export type SkillPackageManifest = V731dSafetyBase & {
  manifest_id: string;
  package_id: string;
  package_name: string;
  package_version: string;
...
export type SkillPackageSourceTraceBundle = V731dSafetyBase & {
  source_trace_bundle_id: string;
  package_id: string;
  source_trace_ids: string[];
  source_experience_ids: string[];
  source_draft_id: string;
...
export type SkillPackageAuditEvent = {
  ...
  timestamp: string;
  metadata_only: boolean;
  system_validation_event: boolean;
  real_training_triggered: boolean;
  skill_published: boolean;
};
export type SkillPackageAudit = V731dSafetyBase & {
  package_id: string;
  events: SkillPackageAuditEvent[];
  event_count: number;
  warnings: string[];
};
export type SkillPackageValidationResult = V731dSafetyBase & {
  package_id: string;
  pre_publish_gate_status: string;
  package_status: SkillPackageStatus;
  gate_passed: boolean;
  ready_for_training_package_build: boolean;
  validation_errors: string[];
  warnings: string[];
};
export type SkillPackage = V731dSafetyBase & {
  package_id: string;
  package_name: string;
  package_version: string;
  source_draft_id: string;
  ...
  audit_events: SkillPackageAuditEvent[];
  validation_result?: SkillPackageValidationResult | null;
  warnings: string[];
};
export type SkillPackageList = V731dSafetyBase & {
  skill_packages: SkillPackage[];
  package_count: number;
  system_validated_count: number;
  warnings: string[];
};
export type SkillPackageBuildResponse = V731dSafetyBase & { skill_package: SkillPackage; packaged_experience_count: number; warnings: string[] };
export type V731dSkillPackagePipelineStatus = V731dSafetyBase & Record<string, unknown> & {
  status: string;
  package_count: number;
  system_validated_count: number;
  warnings: string[];
};
export type ExperiencePackageStatus =
  | "training_task_created"
  | "training_completed"
  | "experience_package_built"
  | "pending_practice_load_review";
export type V731eSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  local_private_processing_only: boolean;
  system_validated_package_required: boolean;
  approved_experience_only: boolean;
  redacted_output_only: boolean;
  abstracted_experience_only: boolean;
  structured_training_metadata_only: boolean;
  training_output_manual_review_status: string;
  practice_load_review_status: string;
  source_trace_required: boolean;
  audit_required: boolean;
  provider_call_executed: boolean;
  ...
  source_content_returned: boolean;
  source_material_returned: boolean;
  non_validated_package_used: boolean;
  unreviewed_experience_used: boolean;
  real_codex_training_triggered: boolean;
  formal_training_set_written: boolean;
  skill_published: boolean;
  skill_publishable: boolean;
...
export type TrainingTaskBuildRequest = {
  source_skill_package_id?: string | null;
  task_name: string;
  explicit_system_validated_package_confirmation: boolean;
  explicit_no_manual_training_output_review_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
  explicit_no_real_training_confirmation: boolean;
  explicit_no_skill_publish_confirmation: boolean;
};
export type ExperiencePackageBuildRequest = {
  source_training_task_id?: string | null;
  source_skill_package_id?: string | null;
  package_name: string;
  package_version: string;
  explicit_pending_practice_load_review_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
  explicit_no_real_training_confirmation: boolean;
  explicit_no_skill_publish_confirmation: boolean;
};
export type TrainingSample = V731eSafetyBase & {
  sample_id: string;
  ...
  expected_output_metadata: Record<string, unknown>;
  input_output_pair_id: string;
  sample_status: string;
  created_at: string;
};
export type TrainingPackageAuditEvent = {
  event_id: string;
  artifact_id: string;
  action: string;
  timestamp: string;
...
export type TrainingPackageSourceTraceBundle = V731eSafetyBase & {
  source_trace_bundle_id: string;
  artifact_id: string;
  source_skill_package_id: string;
  source_training_task_id?: string | null;
...
export type TrainingTask = V731eSafetyBase & {
  training_task_id: string;
  task_name: string;
  source_skill_package_id: string;
  source_draft_id: string;
  sample_count: number;
  training_task_status: string;
  lifecycle_statuses: string[];
  samples: TrainingSample[];
  audit_events: TrainingPackageAuditEvent[];
  created_at: string;
  updated_at: string;
  warnings: string[];
};
export type TrainingTaskList = V731eSafetyBase & { training_tasks: TrainingTask[]; task_count: number; training_completed_count: number; warnings: string[] };
export type TrainingTaskBuildResponse = V731eSafetyBase & { training_task: TrainingTask; sample_count: number; warnings: string[] };
export type InternalExperiencePackage = V731eSafetyBase & {
  package_id: string;
  package_name: string;
  package_version: string;
  source_training_task_id: string;
  source_skill_package_id: string;
  source_draft_id: string;
  source_experience_ids: string[];
  training_task_status: string;
  build_status: string;
  package_status: ExperiencePackageStatus;
  experience_package_status: ExperiencePackageStatus;
  sample_count: number;
  samples: TrainingSample[];
...
export type ExperiencePackageList = V731eSafetyBase & { training_packages: InternalExperiencePackage[]; package_count: number; pending_practice_load_review_count: number; warnings: string[] };
export type ExperiencePackageBuildResponse = V731eSafetyBase & { training_package: InternalExperiencePackage; sample_count: number; warnings: string[] };
export type TrainingPackageAudit = V731eSafetyBase & { package_id: string; events: TrainingPackageAuditEvent[]; event_count: number; warnings: string[] };
export type V731eTrainingPipelineStatus = V731eSafetyBase & Record<string, unknown> & {
  status: string;
  training_task_count: number;
  training_package_count: number;
  pending_practice_load_review_count: number;
  warnings: string[];
};

export type PracticeLoadReviewStatus =
  | "pending_practice_load_review"
  | "review_editing"
  ...
  | "changes_requested";
export type V731fSafetyBase = Record<string, unknown> & {
  owner_only: boolean;
  local_private_processing_only: boolean;
  generated_package_read_only: boolean;
  lawyer_review_required: boolean;
  practice_load_review_required: boolean;
  approved_for_practice_load_required: boolean;
  redacted_output_only: boolean;
  abstracted_experience_only: boolean;
  source_trace_required: boolean;
  audit_required: boolean;
  sensitive_field_scan_required: boolean;
  provider_call_executed: boolean;
  key_value_read: boolean;
  credential_value_returned: boolean;
  provider_result_payload_returned: boolean;
  source_content_returned: boolean;
  source_material_returned: boolean;
  unreviewed_package_loaded: boolean;
  unredacted_content_loaded: boolean;
  non_pending_review_package_loaded: boolean;
  missing_source_trace_loaded: boolean;
  real_codex_training_triggered: boolean;
  formal_training_set_written: boolean;
  skill_updated: boolean;
  skill_published: boolean;
...
export type ExperienceCard = {
  ...
  source_experience_id: string;
  title: string;
  generated_experience_text: string;
  lawyer_experience_text: string;
  applicable_scenarios: string[];
  not_applicable_scenarios: string[];
  risk_warnings: string[];
  usage_boundaries: string[];
  gray_load_enabled: boolean;
  review_status: PracticeLoadReviewStatus;
  metadata_safe: boolean;
};
export type ExperienceCardEdit = {
  card_id: string;
  title: string;
  lawyer_experience_text: string;
  applicable_scenarios: string[];
...
export type PracticeLoadReviewEditRequest = {
  reviewer_id: string;
  reviewer_note: string;
  explicit_lawyer_review_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
...
export type PracticeLoadReviewSaveRequest = PracticeLoadReviewEditRequest & {
  edited_cards: ExperienceCardEdit[];
  gray_load_enabled: boolean;
};
export type PracticeLoadReviewDecisionRequest = {
  reviewer_id: string;
  reviewer_note: string;
  gray_load_enabled: boolean;
  explicit_lawyer_review_confirmation: boolean;
  explicit_system_revalidated_confirmation: boolean;
  explicit_no_provider_confirmation: boolean;
  explicit_no_real_training_confirmation: boolean;
  explicit_no_skill_publish_confirmation: boolean;
};
export type PracticeLoadReviewAuditEvent = {
  event_id: string;
  package_id: string;
  action: string;
  timestamp: string;
  ...
  real_training_triggered: boolean;
  skill_published: boolean;
  practice_runtime_loaded: boolean;
};
export type PracticeLoadReviewSourceTraceBundle = V731fSafetyBase & {
  source_trace_bundle_id: string;
  package_id: string;
  source_training_package_id: string;
  source_training_task_id: string;
  source_skill_package_id: string;
  source_trace_ids: string[];
  source_experience_ids: string[];
...
export type PracticeLoadRevalidationResult = V731fSafetyBase & {
  package_id: string;
  validation_status: PracticeLoadReviewStatus;
  revalidation_passed: boolean;
  all_cards_metadata_safe: boolean;
  source_trace_complete: boolean;
  audit_complete: boolean;
  sensitive_field_scan_passed: boolean;
  generated_package_preserved: boolean;
  lawyer_approved_package_ready: boolean;
  validation_errors: string[];
  warnings: string[];
};
export type PracticeLoadReviewPackage = V731fSafetyBase & {
  package_id: string;
  source_training_package_id: string;
  source_training_task_id: string;
  source_skill_package_id: string;
  ...
  package_version: string;
  review_status: PracticeLoadReviewStatus;
  load_gate_status: PracticeLoadReviewStatus;
  validation_status: PracticeLoadReviewStatus;
  can_load_to_practice_runtime: boolean;
  gray_load_enabled: boolean;
  generated_experience_package: Record<string, unknown>;
  lawyer_approved_experience_package?: Record<string, unknown> | null;
  experience_cards: ExperienceCard[];
  source_trace_bundle: PracticeLoadReviewSourceTraceBundle;
  audit_events: PracticeLoadReviewAuditEvent[];
  revalidation_result?: PracticeLoadRevalidationResult | null;
  created_at: string;
  updated_at: string;
  warnings: string[];
};
export type PracticeLoadReviewPackageList = V731fSafetyBase & {
  packages: PracticeLoadReviewPackage[];
  package_count: number;
  pending_practice_load_review_count: number;
  approved_for_practice_load_count: number;
  warnings: string[];
};
export type PracticeLoadReviewPackageAudit = V731fSafetyBase & { package_id: string; events: PracticeLoadReviewAuditEvent[]; event_count: number; warnings: string[] };
export type V731fPracticeLoadPipelineStatus = V731fSafetyBase & Record<string, unknown> & {
  status: string;
  package_count: number;
  pending_practice_load_review_count: number;
  system_revalidated_count: number;
  approved_for_practice_load_count: number;
  rejected_for_practice_load_count: number;
  warnings: string[];
};
