...
export default function PersonalCaseProductionPage() {
  ...
  async function loadWorkflow() {
    ...
    try {
      ...
      setError("真实案件生产工作流 API 暂不可用，请确认后端服务已启动。");
    }
  }
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        <Hero title="真实案件生产工作流" subtitle="把材料处理、AI 草稿、信息核验与律师复核串联为受控生产流程" badges={["受控编排", "草稿优先", "律师复核必需", "不自动对外交付"]} />
        ...
        <Panel title="Workflow Stage Cards">
        ...
        <Panel title="Review Gate Queue"><FormGrid><Select value={reviewAction} options={reviewActions} onChange={setReviewAction} /><Button label="提交复核门禁动作" onClick={() => void submitReview()} /></FormGrid></Panel>
        <Panel title="Source Trace"><div className="grid gap-3 md:grid-cols-3">{(data.traces?.source_traces ?? []).slice(0, 6).map((trace: any) => <Card key={trace.source_trace_id} title={trace.source_trace_id} lines={[trace.source_type, trace.source_label, `raw=${trace.raw_content_returned}`]} />)}</div></Panel>
        <Panel title="安全清单"><div className="grid gap-2">{(data.safety?.safety_checklist ?? []).map((item: string) => <Row key={item} label={item} value="通过" />)}</div></Panel>
        <Panel title="Developer Diagnostics"><details className="rounded-md border border-line bg-slate-950 text-slate-100"><summary className="cursor-pointer px-4 py-3 text-sm">JSON metadata</summary><pre className="max-h-96 overflow-auto p-4 text-xs">{JSON.stringify(data, null, 2)}</pre></details></Panel>
      </div>
    </AppShell>
  );
}
