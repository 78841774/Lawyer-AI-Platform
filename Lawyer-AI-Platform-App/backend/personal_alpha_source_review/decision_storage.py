...
RUNTIME_DECISION_STORAGE_RELATIVE_PATH = "storage/runtime/personal_alpha_source_review/decisions"
REPO_ROOT = Path("/Users/wazhen/Lawyer-AI-Platform")


def append_source_review_decision(workspace_run_id: str, record: dict[str, Any]) -> dict[str, Any]:
...
def list_source_review_decisions(workspace_run_id: str) -> list[dict[str, Any]]:
    safe_run_id = _safe_id(workspace_run_id, "workspace_run_missing")
    return _read_decisions_from_file(_decision_storage_file(safe_run_id))


def _decision_storage_file(workspace_run_id: str) -> Path:
    return REPO_ROOT / RUNTIME_DECISION_STORAGE_RELATIVE_PATH / f"{workspace_run_id}.json"
