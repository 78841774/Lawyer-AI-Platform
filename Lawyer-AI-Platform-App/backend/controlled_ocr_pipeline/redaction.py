...
MAX_REDACTED_OCR_PREVIEW_CHARS = 8000
...
def build_redacted_ocr_preview(raw_ocr_text: str) -> dict[str, Any]:
    redacted = raw_ocr_text
    detected: list[str] = []
    ...
    truncated = False
    if len(redacted) > MAX_REDACTED_OCR_PREVIEW_CHARS:
        redacted = redacted[:MAX_REDACTED_OCR_PREVIEW_CHARS]
        truncated = True
    warnings = ["OCR redaction is best-effort and requires manual lawyer review."]
    if truncated:
        warnings.append("Redacted OCR preview was truncated to 8000 characters.")
    return {
        "redacted_ocr_preview": redacted,
