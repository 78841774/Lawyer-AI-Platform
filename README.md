...
Team Workspace deferred / 团队版后置。External Client Delivery deferred / 外部交付后置。当前个人版本地连续大阶段已经包含 v7.25-v7.31a 未提交实现：实战试运行准备、真实接口接入准备、OCR / 文档接口受控接入、个人生产统一受控接口接入总控台、法律检索与企业信息 API 受控接入、Codex 训练方案与多层级案由训练产物加载器、已结案件 Codex 训练执行、真实已结案件训练材料导入与脱敏管线。大阶段仍等待统一验证与用户确认后再 commit / tag / push / release。
...
The next planned sub-stage is v7.31b Codex Training on Real Closed Cases. Codex training remains metadata artifact generation/loading, not fine-tuning model parameters.
