...
Team Workspace deferred / 团队版后置。External Client Delivery deferred / 外部交付后置。当前个人版连续大阶段已经包含 v7.25-v7.38：实战试运行准备、真实接口接入准备、OCR / 文档接口受控接入、个人生产统一受控接口接入总控台、法律检索与企业信息 API 受控接入、Codex 训练方案与多层级案由训练产物加载器、已结案件 Codex 训练执行、真实已结案件训练材料导入与脱敏管线、受控经验候选管线、Skill Experience Pool 与 Codex Skill 草案生成器、Skill Package 版本化封装与系统校验、Internal Training / Experience Package Builder、Practice Runtime Load Review Gate、Practice Runtime Controlled Loading & Monitoring、Practice Runtime Output Observation & Lawyer Feedback Loop、Practice Feedback Candidate Pack、Next Experience Package Rebuild、Experience Lifecycle Consolidation、Case Analysis Skill Output Schema Driven Workbench、Case Analysis Output Feedback to Experience Improvement Candidate、Training Dataset Builder & Training Gate、未脱敏基础材料受控解析、OCR / 法律检索校验、脱敏经验包生成、Codex Skill Training Dry Run、Safe Callable Training Skill Spec、Codex Skill Internal Training Run 与 v7.38 安全 Skill 内部训练链路。v7.34-v7.37 已在 `v7.37` 标签目标提交完成，v7.35/v7.36/v7.37 标签指向该连续阶段提交；v7.35a-v7.38 当前为本地未提交工作区实现，尚未做发布处理。
...
## Safe Callable Training Skill Chain

v7.35a-v7.38 adds the corrected raw-material-to-safe-training-skill chain inside `/personal-skill-studio/training-artifacts`: controlled training-material parsing metadata, OCR/document parse checks, legal retrieval checks, redacted experience package generation, safe callable Training Skill Spec, and v7.38 internal simulation artifact connection. The generated experience package includes legal summary, common fact extraction framework, case-cause-specific fact points, fact summary, `substantive_experience_profiles`, `procedural_experience_profiles`, two-layer loading contract, diff readiness, audit, and source trace metadata.

Substantive experience may be aggregated across procedures only as bounded `substantive_reference` with source-stage labels. Procedural experience is `procedural_exact_match` only and must not cross procedure or stage.

v7.35a-v7.38 does not store credential values in Skills, does not return source payloads, OCR content, party identity, filesystem locations, provider payloads, or credential values, does not call live providers, does not publish Skills, does not replace runtime packages, and does not trigger external delivery.

## Codex Panel Training Skill

v7.38b adds `.agents/skills/lawyer-ai-training-skill/` so Codex panel / agent sessions can load the same training workflow rules used by the local development process.

This Codex Panel Training Skill is different from the business-system Training Skill Spec generated under `/personal-skill-studio/training-artifacts/codex-training-skills/*`. The business Spec is a metadata artifact for training outputs; the panel Skill is a Codex-facing workflow guide with resources and helper scripts for development, dry-run checks, safety checks, and regression validation.

The panel Skill does not contain real credentials, real case materials, OCR full text, provider raw responses, local absolute paths, publishable runtime Skills, or runtime package replacement logic. Future Codex training tasks should explicitly load `lawyer-ai-training-skill` or read `.agents/skills/lawyer-ai-training-skill/SKILL.md` first.

## Next Step

The implementation is complete through v7.38b in the local worktree. The next recommended step is user-confirmed release handling or an explicitly scoped next personal-version sub-stage. Codex training remains metadata artifact generation/loading, not automatic public Skill publishing or external model training.
